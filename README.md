...
`orth` is a shell script that provides a set of commands for working with a specific scan result. Thus, the first step
is to initialize an arbitrary empty directory with a scan result using the `orth init` command. After that, you
can issue further commands: run `orth` without any arguments to see all the available options.

To fix issues with a scan result, you will most likely need to set entries in the local and global configuration,
i.e. the file `.ort.yml` and the files within the `configuration repository`, respectively. All `orth` commands use the
...
configuration change and then regenerate all reports with the command `orth report` or you can recreate a specific
report, with a command like `orth report-html`, which usually takes a few seconds to a minute.

Besides generating `ORT` reports `orth` provides many other helpful commands, e.g. for investigating license
...
The following sections explain the use of some of the commands and illustrate suggested `orth`-based workflows. The
...
2. Put `orth` on the system path, e.g. create a symbolic link to `orth` in the `bin` directory (alternatively copy
   `orth` to `bin`, but remember to do that again after each update)
3. Copy `orthconfig-template` to a file `~/.orthconfig`
4. Clone the OSS repositories `configuration`, `ort`, `scancode-toolkit` and set the parameters in
   `~/.orthconfig` to point to those local clones and to `support` (this repository)
5. Build `ORT` (`ort` repository cloned in the previous step) via `./gradlew installDist`
4. Verify that `orth` works by proceeding with the following two sections
...
orth analyze <source-code-dir>            # To run the analyzer without Docker.
orth analyze-in-docker <source-code-dir>  # To run the analyzer in the Docker container.
```
...
orth init https://example.com/scans/123456/scan-result.json
cat ort.yml
...
```bash
orth report          # Compute all reports like static HTML, Web-App or a `FOSS_NOTICE` file.
orth report-html     # Compute only the static HTML report.
orth report-*        # Compute any other single report, see also list of available commands.
```
...
```bash
orth rc-clean <source-code-dir>
```

#### Generating scope excludes

```bash
orth rc-generate-scope-excludes
```
...
```bash
orth rc-import-path-excludes <source-code-dir>
...
orth rc-export-path-excludes
```
...
```bash
orth rc-import-curations
...
orth rc-export-curations
```
...
1. grouped by license text
  ...
  orth licenses Unmanaged:manifest.xml:manifest:91a99da65c0e1558085b01b0de44ddf38e376a48 ~/sources/repo/manifest/
  # Slow variant as sources are downloaded:
  orth licenses Unmanaged:manifest.xml:manifest:91a99da65c0e1558085b01b0de44ddf38e376a48
  ```
...
1. grouped by license text
  ```bash
  # Fast variant as sources are provided (revision needs to match scanned revision):
  orth offending-licenses Unmanaged:manifest.xml:manifest:91a99da65c0e1558085b01b0de44ddf38e376a48 ~/sources/repo/manifest/
  # Slow variant as sources are downloaded:
  orth offending-licenses Unmanaged:manifest.xml:manifest:91a99da65c0e1558085b01b0de44ddf38e376a48
  ```
2. not grouped
  ```bash
  orth offending-licenses Unmanaged:manifest.xml:manifest:91a99da65c0e1558085b01b0de44ddf38e376a48 .
...
```bash
orth rc-generate-timeout-error-resolutions
```
...
```bash
orth export-copyright-garbage
```   

## An `orth` based workflow for OSS reviews
...
3. Fix offending licenses in C/C++ sources (and dependency sources present in the HERE source tree)
  1. Iterate over the output of `orth offending licenses` and add path excludes if applicable
  2. Iterate over the output of `orth offending licenses` and add license finding curations if applicable
...
4. Fix offending licenses in dependencies (managed by a package manager)
  ...
  2. Regenerate the report to see the effect via `orth report-html`
...
6. Fix the FOSS_NOTICE file
  ...
  3. Recompute the FOSS_NOTICE file via `orth report-notices` to see the effect

Tips:
* Make the `ort.yml` in your `orth` directory a symlink to a VCS-managed file so that you can commit your changes often
