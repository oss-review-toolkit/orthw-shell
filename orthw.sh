...
list_scan_results() {
  ...
  local sql="SELECT ROW_NUMBER() OVER (ORDER BY identifier) as index,identifier FROM scan_results WHERE identifier LIKE '$package_id'"
...
if [ "$command" = "report" ] && [ "$#" -eq 1 ]; then
  require_initialized
  report "EvaluatedModel,GitLabLicenseModel,PlainTextTemplate,SpdxDocument,StaticHtml,WebApp"
  exit 0
fi
